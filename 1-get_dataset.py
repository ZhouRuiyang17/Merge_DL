...
def generate_dataset_and_index(out_dir):
    """
    ...
    ### 按照天来处理，减少内存占用
    for date in dates:
        df_date = df.loc[date:date + pd.Timedelta(hours=23, minutes=59)]
        rows = []
        dataset = []
        for timestamp in df_date.index:
            all_griddata = []
            ### 处理每个雷达和雨量站
        ...
        index_dataset.to_csv(f'{out_dir}/index_dataset-ACC1H-hsr-{date.strftime("%Y%m%d")}-W{WINDOW_SIZE}.csv', index=False)
        dataset = np.stack(dataset, axis=0)  # (N_samples, N_radars*5+1, WINDOW_SIZE, WINDOW_SIZE)
        # np.savez_compressed(f'./dataset/dataset-ACC1H-hsr-{date.strftime("%Y%m%d")}-W{WINDOW_SIZE}.npz', dataset=dataset)
        np.save(f'{out_dir}/dataset-ACC1H-hsr-{date.strftime("%Y%m%d")}-W{WINDOW_SIZE}.npy', dataset)
...
generate_dataset_and_index(out_dir="./dataset_files")
# 用法：
# df = pd.read_csv('dataset/filelist-ACC1H-hsr-2019.csv', index_col=0, parse_dates=True)
# dict_gridinfo = {radarname: np.load(f'gridinfo/Merge_DL-0.001deg/gridinfo_{radarname}.npz') for radarname in df.columns[:-1]}
